...
if (typeof DOSM.Metadata == "undefined") {
  DOSM.Metadata = {};
} // Store temporarily the retrieved Metadata
if (typeof DOSM.Utilities == "undefined") {
  DOSM.Utilities = {};
} // Utilities Functions
// #endregion
...
DOSM.Utilities.GetArrayFromProperty = function (records, propertyName) {
  ...
  records.forEach(function (record) {
    if (
      record.hasOwnProperty(propertyName) &&
      values.indexOf(record[propertyName]) == -1
    ) {
      values.push(record[propertyName]);
    }
  });
  return values;
};
...
DOSM.Utilities.SortArrayBySortType = function (array, sort) {
  switch (sort) {
    case "default_desc":
      ...
      break;
    case "label":
      array.sort(DOSM.Utilities.CustomSort("text"));
      break;
    case "label_desc":
      array.sort(DOSM.Utilities.CustomSort("-text"));
      break;
    case "value":
      array.sort(DOSM.Utilities.CustomSort("value"));
      break;
    case "value_desc":
      array.sort(DOSM.Utilities.CustomSort("-value"));
      break;
...
DOSM.Utilities.GetUniqueEntriesFromArrays = function (arrays) {
  ...
  allValues.forEach(function (value) {
    ...
    if (valueFound == true && uniqueEntries.indexOf(value) == -1) {
      ...
      if (this.status == 200) {
        ...
        try {
          ...
          DOSM.Metadata.Dependencies.forEach(function (dependency) {
            ...
            if (DOSM.Utilities.HasValue(sortedFields[dependency.parent])) {
              dependency.parent_sort = sortedFields[dependency.parent];
            } else {
              if (!DOSM.Utilities.HasValue(dependency.parent_sort)) {
                dependency.parent_sort = "default";
              }
              sortedFields[dependency.parent] = dependency.parent_sort;
            }

            // check dependency.child
            if (DOSM.Utilities.HasValue(sortedFields[dependency.child])) {
              dependency.child_sort = sortedFields[dependency.child];
            } else {
              if (!DOSM.Utilities.HasValue(dependency.child_sort)) {
                dependency.child_sort = "default";
              }
            ...
            if (parentField != null) {
              dependency.parent_values = parentField.getOptions();
            }
            DOSM.Utilities.SortArrayBySortType(
              dependency.parent_values,
              dependency.parent_sort
            );
            ...
            if (childField != null) {
              dependency.child_values = childField.getOptions();
            }
            DOSM.Utilities.SortArrayBySortType(
              dependency.child_values,
              dependency.child_sort
            );

            if (allParents.indexOf(dependency.parent) == -1) {
              allParents.push(dependency.parent);
            }
            if (allChildren.indexOf(dependency.child) == -1) {
              allChildren.push(dependency.child);
            }

            if (allFields.indexOf(dependency.parent) == -1) {
              allFields.push(dependency.parent);
            }
            if (allFields.indexOf(dependency.child) == -1) {
              allFields.push(dependency.child);
            }
          });
          ...
          allParents.forEach(function (parentCheck) {
            if (
              allChildren.indexOf(parentCheck) == -1 &&
              onlyParents.indexOf(parentCheck) == -1
            ) {
              onlyParents.push(parentCheck);
            }
          });
          ...
          onlyParents.forEach(function (onlyParent) {
            DOSM.Metadata.Dependencies.forEach(function (dependency) {
              if (
                dependency.parent == onlyParent &&
                onlyParents.indexOf(dependency.parent) > -1 &&
                sortedOnlyParents.indexOf(dependency.parent) == -1
              ) {
                ...
                if (onlyParentField != null) {
                  ...
                  onlyParentField.controls.forEach(function (control) {
                    ...
                    dependency.parent_values.forEach(function (sortedOption) {
                      if (sortedOption.value != -1) {
                        control.addOption(
                          onlyParentField.getOption(
                            parseInt(sortedOption.value)
                          )
                        );
                      }
                    });
          ...
          Xrm.Navigation.openAlertDialog({
            text:
              "Failed to parse configuration data for Web Resource " +
              webResourcePath,
          });
        }
      } else {
        ...
        Xrm.Navigation.openAlertDialog({
          text:
            "Failed to load configuration data for Web Resource " +
            webResourcePath,
        });
      }
    }
...
DOSM.FilterField = function (
  ...
  if (DOSM.Metadata.Dependencies) {
    DOSM.Metadata.Dependencies.forEach(function (dependency) {
      if (dependency.child == childFieldName) {
    ...
    DOSM.Metadata.Dependencies.forEach(function (dependency) {
      if (
        dependency.parent == parentFieldName &&
        dependency.child == childFieldName
      ) {
        ...
        childDependencies.forEach(function (childDependency) {
          var otherParentField = formContext.getAttribute(
            childDependency.parent
          );
            ...
            if (otherParentFieldValue != null && otherParentFieldValue != -1) {
              ...
              if (Array.isArray(otherParentFieldValue)) {
                ...
                otherParentFieldValue.forEach(function (otherParentValue) {
                  childDependency.options[otherParentValue.toString()].forEach(
                    function (otherMultiValue) {
                      if (
                        otherValidOptionValues.indexOf(otherMultiValue) == -1
                      ) {
                        otherValidOptionValues.push(otherMultiValue);
                      }
                    }
                  );
                });
                ...
                childrenValues.push(
                  childDependency.options[otherParentFieldValue.toString()]
                );
              }
            }
          }
        ...
        var validOptionValues =
          DOSM.Utilities.GetUniqueEntriesFromArrays(childrenValues);
        ...
        if (validOptionValues.length == 0) {
          ...
          var sortedChildValues = DOSM.Utilities.GetArrayFromProperty(
            dependency.child_values,
            "value"
          ).map(String);
          validOptionValues.sort(function (a, b) {
            return sortedChildValues.indexOf(a) - sortedChildValues.indexOf(b);
          });
          ...
          childField.controls.forEach(function (control) {
            ...
            validOptionValues.forEach(function (validOptionValue) {
              if (childField.getOption(parseInt(validOptionValue))) {
                control.addOption(
                  childField.getOption(parseInt(validOptionValue))
                );
              }
            });
          });
          ...
          if (currentChildFieldValue != null) {
            if (Array.isArray(currentChildFieldValue)) {
              ...
              currentChildFieldValue.forEach(function (currentValue) {
                if (
                  validOptionValues.indexOf(currentValue.toString()) > -1 &&
                  validMultiValues.indexOf(currentValue) == -1
                ) {
                  validMultiValues.push(currentValue);
                }
              });
              ...
              if (
                validOptionValues.indexOf(currentChildFieldValue.toString()) >
                -1
              ) {
